...
void MainInterface::connectManagement()
{
    try
    {
        ...
        // {{{ CONNECT_REQUEST (0x0205)
        std::vector<uint8_t> data{ 0x06, 0x10, 0x02, 0x05, 0x00, 0x18, 0x08, 0x01, _listenIpBytes[0], _listenIpBytes[1], _listenIpBytes[2], _listenIpBytes[3], _listenPortBytes[0], _listenPortBytes[1], 0x08, 0x01, _listenIpBytes[0], _listenIpBytes[1], _listenIpBytes[2], _listenIpBytes[3], _listenPortBytes[0], _listenPortBytes[1], 0x02, 0x03 };
        std::vector<uint8_t> response;
        getResponse(ServiceType::CONNECT_RESPONSE, data, response);
        if(response.size() < 18)
        {
