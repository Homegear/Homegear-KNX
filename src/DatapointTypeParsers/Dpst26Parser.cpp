...
void Dpst26Parser::parse(BaseLib::SharedObjects* bl, const std::shared_ptr<BaseLib::DeviceDescription::Function>& function, const std::string& datapointType, uint32_t datapointSubtype, std::shared_ptr<BaseLib::DeviceDescription::Parameter>& parameter)
{
    ...
    if(datapointType == "DPST-26-1")
    {
        ...
        additionalParameters.push_back(createParameter(function, baseName + ".INACTIVE", "DPT-1", "", IPhysical::OperationType::store, parameter->readable, parameter->writeable, parameter->roles, 1, 1, std::make_shared<BaseLib::DeviceDescription::LogicalBoolean>(GD::bl)));
