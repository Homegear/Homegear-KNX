...
Cemi::Cemi(Operation operation, uint16_t sourceAddress, uint16_t destinationAddress, bool payloadFitsInFirstByte, std::vector<uint8_t>& payload) : _operation(operation), _sourceAddress(sourceAddress), _destinationAddress(destinationAddress), _payloadFitsInFirstByte(payloadFitsInFirstByte), _payload(payload)
{
    _messageCode = 0x11;
	if(_payload.empty())
	{
...
Cemi::Cemi(Operation operation, uint16_t sourceAddress, uint16_t destinationAddress, uint8_t tpduSequenceNumber, bool payloadFitsInFirstByte, std::vector<uint8_t>& payload) : _operation(operation), _sourceAddress(sourceAddress), _destinationAddress(destinationAddress), _numbered(true), _tpduSequenceNumber(tpduSequenceNumber), _payloadFitsInFirstByte(payloadFitsInFirstByte), _payload(payload)
{
    _messageCode = 0x11;
	if(_payload.empty())
	{
